    ...
    .on('data', (row) => {
      ...
      Object.keys(row).forEach(key => {
        ...
        if (['timestamp', 'time'].includes(normalizedKey)) {
          // Try parsing as ISO or HH:mm:ss
          const parsedTime = new Date(row[key]);
          normalizedRow.time = isNaN(parsedTime.getTime())
            ? new Date(`1970-01-01T${row[key]}Z`) // Assume HH:mm:ss
            : parsedTime;
          if (isNaN(normalizedRow.time.getTime())) {
            console.warn('Invalid time format in row:', row);
            return;
          }
          normalizedRow.time = normalizedRow.time.toISOString();
